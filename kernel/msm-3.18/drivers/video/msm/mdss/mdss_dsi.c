 ...
 *
 *
 * Added sn65dsi83_stop_video(void);
 * Added sn65dsi83_start_video(void);
 * Added sn65dsi83_init_video(void);
...
static int sn65dsi83_init_done = 0;

#define DSI_DISABLE_PC_LATENCY 100
#define DSI_ENABLE_PC_LATENCY PM_QOS_DEFAULT_VALUE

extern  int sn65dsi83_stop_video(void);
extern int sn65dsi83_start_video(void);
extern int sn65dsi83_init(void);
extern void switch_backlight(int enable);
...
static int mdss_dsi_off(struct mdss_panel_data *pdata, int power_state)
{
	...
	/*Disabling the backlight of projector */
	pr_debug("Going to disable backlight of projector\n");
	switch_backlight(0);
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	if (0 == sn65dsi83_init_done) {
		if (0 != sn65dsi83_init()) {
			printk ("Failed to initialize SN65DSI83 bridge chip\n");

		} else {
			pr_debug ("DEBUG :  SN65DSI83 bridge chip initialized successfully\n");
			sn65dsi83_init_done = 1;
		}
	}
	...
	if (pdata->panel_info.type == MIPI_CMD_PANEL)
		mdss_dsi_clk_ctrl(ctrl_pdata, ctrl_pdata->dsi_clk_handle,
				  MDSS_DSI_ALL_CLKS, MDSS_DSI_CLK_OFF);
	if (1 == sn65dsi83_init_done) {
		pr_debug ("DEBUG :  Startting video from SN65DSI83 bridge chip\n");
		sn65dsi83_start_video();
		sn65dsi83_init_done++;
	}

	/*Enabling the backlight of projector */
	pr_debug("Going to enable backlight of projector\n");
	switch_backlight(1);
