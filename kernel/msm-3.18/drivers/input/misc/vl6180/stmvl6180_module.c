...
static void stmvl6180_work_handler(void);
...
#endif
        if(data->rangeData.range_mm && (data->rangeData.range_mm < 150)) {
           printk("Sending power key\n");
       	   input_report_key(data->input_dev_ps, KEY_POWER, 1);
	   input_sync(data->input_dev_ps);
           input_report_key(data->input_dev_ps, KEY_POWER, 0);
           input_sync(data->input_dev_ps);
        }
...
static void stmvl6180_work_handler(void)
{
	...
	if (data->enable_ps_sensor == 1) {
		...
		if (data->enableDebug) {
			uint8_t range_status = 0, range_start = 0,interrupt_status=0;			
			VL6180x_RdByte(vl6180x_dev, RESULT_INTERRUPT_STATUS_GPIO, &interrupt_status);
			VL6180x_RdByte(vl6180x_dev, RESULT_RANGE_STATUS, &range_status);
			VL6180x_RdByte(vl6180x_dev, SYSRANGE_START, &range_start);
			vl6180_errmsg("Interrupt status is 0x%x,RangeStatus as 0x%x, range_start: 0x%x\n",
							interrupt_status,range_status, range_start);
		}
#if 0
        ...
        ret = VL6180x_RangeSetInterMeasPeriod(vl6180x_dev,10);
        ret = VL6180x_SetupGPIO1(vl6180x_dev,1,GPIOx_SELECT_GPIO_INTERRUPT_OUTPUT);
        ret = VL6180x_RangeConfigInterrupt(vl6180x_dev,CONFIG_GPIO_INTERRUPT_LEVEL_LOW);
        ret = VL6180x_RangeStartContinuousMode(vl6180x_dev);
#endif 
     
        ret = VL6180x_RangeGetMeasurement(vl6180x_dev, &(data->rangeData));
        if (!ret && data->rangeData.errorStatus !=DataNotReady ) {
              vl6180_errmsg("Error Status is 0x%x\n",data->rangeData.errorStatus); 
              stmvl6180_ps_read_measurement(); /* to update data */
        }
        VL6180x_ClearAllInterrupt(vl6180x_dev);
...
static DEVICE_ATTR(enable_ps_sensor, S_IRUGO|S_IWUSR /* 0660 *//*S_IWUGO | S_IRUGO */,
...
static int stmvl6180_start(struct stmvl6180_data *data, uint8_t scaling, init_mode_e mode)
{
	...
	//VL6180x_RangeSetSystemMode(vl6180x_dev, MODE_START_STOP|MODE_SINGLESHOT);
        ...
        //VL6180x_SetOffsetCalibrationData(vl6180x_dev, 0x0A);
        //VL6180x_SetXTalkCompensationRate(vl6180x_dev, 0x9A);

                /*Cross talk compensation settings*/
        VL6180x_WrWord(vl6180x_dev,0x001e,0x9a);    /*Set cross talk compensation rate*/
        VL6180x_WrByte(vl6180x_dev,0x0021,0x0a);    /*Set cross talk valid height*/
        VL6180x_WrWord(vl6180x_dev,0x0022,0x02);    /*Set early convergence estimate*/
        VL6180x_WrByte(vl6180x_dev,0x002d,0x10|0x03); /*Set range check enables*/

        VL6180x_WrByte(vl6180x_dev,0x0024,0x0a);   /*Set part to part range offset*/
        VL6180x_WrByte(vl6180x_dev,0x0025,0xff);   /*Ignore valid height*/
        VL6180x_WrWord(vl6180x_dev,0x0026,0xB8);   /*Set Range Ignore Threshold*/
        VL6180x_WrByte(vl6180x_dev,0x001c,0x07);   /*Set Max Convergence time*/
        VL6180x_WrByte(vl6180x_dev,0x010a,0x41);   /*Average Sampling Period*/
        VL6180x_WrWord(vl6180x_dev,0x0040,0x32);   /*ALS Ingeration Period*/


        VL6180x_RangeSetInterMeasPeriod(vl6180x_dev,1540);
        VL6180x_SetupGPIO1(vl6180x_dev,1,GPIOx_SELECT_GPIO_INTERRUPT_OUTPUT);
        VL6180x_SetGPIOxPolarity(vl6180x_dev,1,0);
        VL6180x_RangeConfigInterrupt(vl6180x_dev,CONFIG_GPIO_INTERRUPT_LEVEL_LOW);
        VL6180x_RangeSetThresholds(vl6180x_dev,150,160,0);  
        VL6180x_RangeStartContinuousMode(vl6180x_dev);


        VL6180x_RangeGetThresholds(vl6180x_dev,&low,&high);
       	vl6180_errmsg("low threshold as 0x%x, high threshold as: 0x%x\n",
							low,high); 
...
static int stmvl6180_setup(struct stmvl6180_data *data)
{
	...
	/* init interrupt */
        ...
        ret=gpio_request(43, "vl6180_gpio_int");
         vl6180_dbgmsg("gpio_request:%d\n", ret);
	ret = gpio_direction_input(43);
         vl6180_dbgmsg("gpio_direction:%d\n", ret);
	irq = gpio_to_irq(43);
	...
	if (irq < 0) {
		...
		//rc = request_threaded_irq(irq,NULL, stmvl6180_interrupt_handler, IRQF_TRIGGER_FALLING, "vl6180_lb_gpio_int", (void *)data);
		rc = request_threaded_irq(irq,NULL, stmvl6180_interrupt_handler, IRQF_TRIGGER_FALLING | IRQF_ONESHOT , "vl6180_lb_gpio_int", (void *)data);
		if (rc) {
...
#endif
        //set_bit(EV_ABS, data->input_dev_ps->evbit);
        input_set_capability(data->input_dev_ps,EV_KEY,KEY_POWER); 
