 ...
 *
 *
 * Few Chnages have been made with respect to logic
 *
 *
 */
				   ...
				   const struct i2c_device_id *id)
{
	...
	//struct irq_domain *domain;
        ...
        //struct stmvl6180_irq_drv_data *irq_data;
	...
	i2c_setclient((void *)client);
        ...
        irq = of_get_named_gpio(node, "qcom,gpio-proximity", 0);
	if (!gpio_is_valid(irq)) {
		vl6180_dbgmsg("TLMM connect gpio not found\n");
		return -EPROBE_DEFER;
	} else {
		irq = gpio_to_irq(irq);
		if (irq < 0) {
			vl6180_dbgmsg( "Unable to configure irq\n");
			return irq;
		}
		vl6180_dbgmsg("%s: virq = %d\n", __func__, irq);
#if 0
		domain = irq_find_host(client->dev.of_node);
		if (unlikely(!domain)) {
			pr_err("%s: domain is NULL", __func__);
			return -EINVAL;
		}
		irq_data = (struct vl6180_data *)domain->host_data;
#endif
		vl6180_data->irq = irq;
		wmb();
		rc = 0;
