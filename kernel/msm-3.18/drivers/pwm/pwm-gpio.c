		...
		int duty_ns, int period_ns)
{
	...
	if (gpio_data->on_time != duty_ns) {
		gpio_data->on_time = duty_ns;
		gpio_data->off_time = period_ns - duty_ns;
	}

	DBG("gpio_pwm_config period=%d and dutycycle=%d", period_ns, duty_ns);
	return 0;
}
...
static int gpio_pwm_probe(struct platform_device *pdev)
{
	...
	gpio_chip->chip.npwm = NUM_PWM;
	gpio_chip->chip.of_xlate = of_pwm_xlate_with_flags;
	gpio_chip->chip.of_pwm_n_cells = NUM_PWM_CELL;
	...
	for (i = 0; i < gpio_chip->chip.npwm; i++) {
		...
		gpio_direction_output(GPIO_PWM, 1);

		gpiod = gpio_to_desc(GPIO_PWM); 
		if (IS_ERR(gpiod)) {
			ret = PTR_ERR(gpiod);
			if (ret != -EPROBE_DEFER)
				dev_err(&pdev->dev,
